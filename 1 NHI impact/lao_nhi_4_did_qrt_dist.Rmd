...
for (k in 1:length(listu)) {
 ...
 rm$robust_ci <- rm$robust_ci[-1, ]  # Drop 0 as that is not really allowed.
...
getdata <- function(i) {
  # keep what we need
  data_alt %>% 
    # keep treated units and all not yet treated units
    filter(group == i | group == 0 | group > i) %>% 
    # create an indicator for the dataset
    mutate(df = i) %>% 
    # make dummies
    mutate(event = if_else(group == i, event, 0))
}

data.stk <- map_df(group_l, getdata) %>% 
  mutate(p_df = paste(pnum, df)) %>%
  mutate(d_df = paste(dnum, df))
...
for (i in 1:length(outname)) {
  ...
  stacked_dyn <- data.stk %>% do(broom::tidy(
    feols(!!outn ~ facnum + semp + urba + litr + imps + 
            i(event, ref = c(-1)) | dnum^df + time^df,
          data = ., cluster = "p_df"), conf.int = TRUE)) %>%
    filter(grepl("event::", term)) %>%
    mutate(t = as.double(str_replace(term, "event::", ""))) %>%
    dplyr::select(t, estimate, std.error, conf.low, conf.high) %>%
    bind_rows(tibble(t = -1, estimate = 0, std.error = 0, 
                     conf.low = 0, conf.high = 0)) %>%
    arrange(t) %>%
...
for (i in 1:length(outname)) {
  impt_gdn_dyn <- data_alt %>% do(broom::tidy(
    did2s(yname = outname[i],
          first_stage = ~ facnum + semp + urba + litr + imps | dnum + time,
...
write_xlsx(didalt, "./output/sfig_9_did_hetero.xlsx")
...
saiw_att_list <- saiw_att_list %>%
  ...
  rename("oval.est" = "estimate", # new -> old
         "oval.se" = "std.error",
         "oval.pval" = "p.value")
...
write_xlsx(saiw_tidy_frame, "./output/sfig_6_did_poverty.xlsx")
...
# Treatment group is group 4, 5, 6
...
# Treatment group (group 4, 5, 6, 8, 9)
...
# Treatment group (group 4, 5, 6 only)
pnumlist <- unique(data.its$pnum)
pnumlist <- pnumlist[pnumlist != 1 & !(pnumlist %in% c(5, 6, 8, 10, 12, 13, 16))]
