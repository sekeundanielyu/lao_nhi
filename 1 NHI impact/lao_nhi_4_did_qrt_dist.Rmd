...
# Log
logpath <- file.path(getwd(), "log", "lao_nhi_4_did_qrt_dist.log")
...
# B7. Robustness checks (Honest DiD)
<br>
...
glance(vc.clslast)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- vc.clslast", "- att")) %>% put()
vc.clslast %>% put()
warnings()
...
glance(vc.clsant1)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- vc.clsant1", "- att")) %>% put()
vc.clsant1 %>% put()
warnings()
...
glance(uc.clslast)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- uc.clslast", "- att")) %>% put()
uc.clslast %>% put()
warnings()
...
glance(uc.clsant1)
print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
print(paste(outname[i], "- uc.clsant1", "- att")) %>% put()
uc.clsant1 %>% put()
warnings()
...
sep("Varying period, Dynamic, Balanced")
...
for (k in 1:length(attgt_list_v)) {
 print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
 print(paste(outname[i], "-", listv[k], "- dyn_b")) %>% put()
 dyn_v_b[[k]] <- aggte(attgt_list_v[[k]], type = "dynamic", 
                      balance_e = 2, na.rm = TRUE) %>% put()
 print(dyn_v_b[[k]])
 # warnings()
...
sep("Universal (fixed) period, Dynamic, Balanced")
...
for (k in 1:length(attgt_list_u)) {
 print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~") %>% put()
 print(paste(outname[i], "-", listu[k], "- dyn_b")) %>% put()
 ...
 dyn_frame_b <- tidy(dyn_u_b[[k]]) %>%
   mutate(overall.att = dyn_u_b[[k]]$overall.att,
...
# B7. Robustness check: DiD heterogeneity-robust estimators
<br>
...
# ------------------------------------------------------------------------------
# paper: https://www.nber.org/papers/w32054
# https://github.com/hollina/stacked-did-weights/tree/main
...
# ------------------------------------------------------------------------------
# paper: https://www.bu.edu/econ/files/2024/07/two-stage-differences-in-differences.pdf
...
write_xlsx(didalt, "./output/fig_b_07_did_hetero.xlsx")


```

# B6. Subgroup analyses: district-level poverty status
<br>
...
write_xlsx(saiw_tidy_frame, "./output/fig_b_04_did_poverty.xlsx")

```

# B8: Controlled interrupted time series analysis (quarterly)
<br>
...
# Seasonal dummy
qtr <- as.yearqtr(as.character(data.its$quarter), format = "%Y.%q")
data.its$quarter_fac <- factor(format(qtr, "%q"),
                                 levels = 1:4,
                                 labels = c("Q1","Q2","Q3","Q4"))
unique(data.its$quarter_fac)
