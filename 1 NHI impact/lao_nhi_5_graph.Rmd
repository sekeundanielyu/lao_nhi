...
write_xlsx(plot.dyn, paste0("./output/", "fig_3_did_main.xlsx"))

# Plot -------------------------------------------------------------------------

data.uc.cls <- plot.dyn %>% filter(name == "uc.cls")
xl <- range(plot.dyn$event.time) # x-axis length / index

plot <- 
  ggplot(data.uc.cls, aes(x = event.time, y = estimate)) +
...
png(paste0("./graph/", "fig_3_did_main.png"), 
...
write_xlsx(plot.fac, paste0("./output/", "sfig_5_did_facility.xlsx"))

# Plot -------------------------------------------------------------------------

plot.fac <- plot.fac %>% filter(name == "uc.cls")
...
png(paste0("./graph/", "sfig_5_did_facility.png"), 
...
plot.pov <- read_excel("./output/sfig_6_did_poverty.xlsx")
plot.pov <- plot.pov %>%
...
write_xlsx(plot.pov, paste0("./output/", "sfig_6_did_poverty.xlsx"))
...
png(paste0("./graph/", "sfig_6_did_poverty.png"), 
    units = "in", width = 5.1, height = 6, res = 500)
print(plot)
dev.off()
...
write_xlsx(plot.grp, paste0("./output/", "sfig_7_did_group.xlsx"))

# Plot -------------------------------------------------------------------------

plot.grp <- plot.grp %>% filter(name == "uc.cls")
...
png(paste0("./graph/","sfig_7_did_group.png"), 
...
write_xlsx(plot.ajt, paste0("./output/", "sfig_8_did_adjust.xlsx"))
...
png(paste0("./graph/", "sfig_8_did_adjust.png"), 
    units = "in", width = 4.75, height = 7, res = 500)
print(plot)
dev.off()
...
data.htr <- read_excel("./output/sfig_9_did_hetero.xlsx")
...
png(paste0("./graph/", "sfig_9_did_hetro.png"), 
    units = "in", width = 5.1, height = 6, res = 500)
print(plot)
dev.off()
...
plot.cnt <- plot.cnt %>% filter(name == "uc.cls")
...
plot.hon <- var_list_h %>%
  ...
  mutate(
    ...
    type2 = factor(type, levels = c("relative_magnitude", "smoothness"),
                   labels = c("Relative magnitudes (M_bar)",
...
plot.hon <- plot.hon %>% filter(name == "uc.cls")
...
plot <- ggplot(
  ...
  ggh4x::facet_grid2(outcome2 ~ type2, scales = "free", axes = "x", 
                     ...
                     labeller = labeller(type2 = type2_labels)) +
  geom_hline(aes(yintercept = 0), color = "black", linewidth = 0.175) +
